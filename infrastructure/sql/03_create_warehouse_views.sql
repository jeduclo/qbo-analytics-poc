...
SELECT
    ...
    c.country,
    CAST(FORMAT(i.invoice_date, 'yyyy-MM-01') AS DATE) AS revenue_month,
...
GROUP BY
    ...
    c.display_name,
    c.country,
    CAST(FORMAT(i.invoice_date, 'yyyy-MM-01') AS DATE);
GO
...
SELECT
    CAST(FORMAT(payment_date, 'yyyy-MM-01') AS DATE) AS payment_month,
    payment_method,
    SUM(amount)       AS total_received,
    COUNT(payment_id) AS payment_count
FROM qbo.stg_payments
GROUP BY
    CAST(FORMAT(payment_date, 'yyyy-MM-01') AS DATE),
    payment_method;
GO
...
WITH customer_history AS (
    SELECT
        i2.customer_id,
        AVG(CAST(DATEDIFF(day, i2.invoice_date, p2.payment_date) AS FLOAT)) AS avg_days_to_pay,
        AVG(CAST(CASE WHEN i2.status = 'Overdue' THEN 1.0 ELSE 0.0 END AS FLOAT)) AS historical_overdue_rate,
        COUNT(i2.invoice_id)   AS invoice_count,
        SUM(i2.balance)        AS current_balance,
        DATEDIFF(day, MAX(p2.payment_date), GETDATE()) AS days_since_last_payment
...
FROM qbo.stg_invoices i
LEFT JOIN customer_history ch ON i.customer_id = ch.customer_id
LEFT JOIN qbo.macro_indicators mac
    ON mac.indicator_date = CAST(FORMAT(i.invoice_date, 'yyyy-MM-01') AS DATE);
GO
